...
void InternationalStudent::set_country(string _country) { country = _country; }
string Countries[6] = {"canada","china","india","iran","korea"};
...
ostream& operator<<(ostream& outs, Student& stu)
{
	...
	outs << ". Research Score: " << stu.get_res_score();   //puts research score in outs stream (int)
...
ostream& operator<<(ostream& outs, InternationalStudent& stu)
{
	...
	outs << ". Research Score: " << stu.get_res_score();
...
bool DomesticStudent::Check_Valid_Province(const DomesticStudent &stu) 
{
	string province = stu.province;
	to_lowercase(province);
	
	for (int i = 0; i < 13; i++)
	{
		if (!(province.compare(Provinces[i])))
		{
			return 1;
		}
	}
...
bool InternationalStudent::Check_Valid_Country(InternationalStudent &stu, int &fixed) {
	string country = stu.country;
	to_lowercase(country);

	if (!country.compare("idian")) {
		stu.set_country("India");
		fixed++;
		return 1;
	}
	...
	for (int i = 0; i < 13; i++)
	{
		if (!(country.compare(Countries[i])))
		{
			return 1;
		}
	}
